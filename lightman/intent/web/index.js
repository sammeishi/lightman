	...
	const App = {
		...
		methods: {
			...
			applyNewData(asr, ig) {
				...
				// 重新生成颜色
				this.generateIntentColorMap()
			},
			/**
			 * 生成意图颜色映射
			 * */
			generateIntentColorMap() {
				this.colors = new Map()
				const group = [
					// 父意图
					[...new Set(Object.keys(this.intentGroups))],
					// 子意图
					[...(new Set(this.asr.map((item) => item.intent)))]
				]
				// 遍历
				group.forEach((intentList) => {
					intentList = intentList.filter((item) => item)
					intentList = intentList.sort(() => (Math.random() - 0.5))
					let colors = this.generateDistinctColors(intentList.length)
					// 使用HSL颜色模型，在色相环上均匀分布颜色
					intentList.forEach((intent, index) => {
